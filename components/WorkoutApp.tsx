...
function getTips(n: string){const l=n.toLowerCase();for(const[k,t]of Object.entries(TIPS))if(k.split("|").some(kk=>l.includes(kk)))return t;return["Execução limpa sempre antes de aumentar a carga","Respire de forma controlada — não prenda durante o esforço"];}

const beep=()=>{try{const c=new(window.AudioContext||((window as unknown as {webkitAudioContext: typeof AudioContext}).webkitAudioContext))();[0,.28,.56].forEach((t,i)=>{const o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);o.type="sine";o.frequency.value=780+i*120;g.gain.setValueAtTime(0,c.currentTime+t);g.gain.linearRampToValueAtTime(.5,c.currentTime+t+.06);g.gain.exponentialRampToValueAtTime(.001,c.currentTime+t+.45);o.start(c.currentTime+t);o.stop(c.currentTime+t+.45);});}catch{}};

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function dayProg(day: any,st: Record<string,boolean>,di: number){
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  let tot=0,done=0;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  [...p,...c].filter((e: any)=>e.series).forEach((ex: any)=>{const k=ex.ordem||ex.exercicio;tot+=ex.series;for(let i=0;i<ex.series;i++)if(st[`${di}_${k}_${i}`])done++;});
  return{done,tot};
}
...
export default function WorkoutApp(){
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const[nutri,setNutri]=useState<any>(NUTRI);
  ...
  const[expEx,setExpEx]=useState<string|null>(null);
  ...
  useEffect(()=>{
    if(rOn&&rt>0){iv.current=setInterval(()=>{setRt(t=>{if(t<=1){if(iv.current)clearInterval(iv.current);setROn(false);beep();return 0;}return t-1;});},1000);}
    return()=>{if(iv.current)clearInterval(iv.current);};
  },[rOn,rt]);
  useEffect(()=>{chatEnd.current?.scrollIntoView({behavior:"smooth"});},[chat]);

  const startRest=()=>{if(iv.current)clearInterval(iv.current);setRt(gRest);setRTot(gRest);setROn(true);};
  const stopRest=()=>{if(iv.current)clearInterval(iv.current);setROn(false);setRt(0);};
  const markSerie=(di: number,exKey: string,i: number)=>{const k=`${di}_${exKey}_${i}`;if(st[k])return;setSt(p=>({...p,[k]:true}));startRest();};
  ...
  const sendChat=async()=>{
    ...
    try{
      ...
      let geminiKey:string|undefined, orKey:string|undefined, orModel:string|undefined;
      if(cryptoKey){
        const encGemini=localStorage.getItem("gym_gemini_key");
        const encOr=localStorage.getItem("gym_or_key");
        if(encGemini)geminiKey=await decryptText(encGemini,cryptoKey);
        if(encOr)orKey=await decryptText(encOr,cryptoKey);
        orModel=localStorage.getItem("gym_or_model")||undefined;
      }

      if(!geminiKey&&!orKey){
        setChat(p=>[...p,{role:"assistant",content:"Nenhuma chave de API configurada. Vá em ⚙️ Configurações e adicione sua chave do Gemini ou OpenRouter."}]);
        setChatLd(false);
        return;
      }
      ...
      const res=await fetch("/api/chat",{
        ...
        body:JSON.stringify({
          messages:[...chat.map(m=>({role:m.role as "user"|"assistant",content:m.content})),{role:"user" as const,content:msg}],
          systemPrompt:sys,
          geminiKey,
          openrouterKey:orKey,
      ...
      if(!res.ok){
        setChat(p=>[...p,{role:"assistant",content:`Erro: ${d.error||"falha na requisição"}`}]);
      }else{
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const principal=Array.isArray(selDay?.bloco?.principal)?selDay.bloco.principal as any[]:[];
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const core=Array.isArray(selDay?.bloco?.core)?selDay.bloco.core as any[]:[];
  const isRun=!!(selDay&&!Array.isArray(selDay?.bloco?.principal)&&selDay?.bloco?.principal?.tipo);

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const ExCard=({ex,di}:{ex:any,di:number})=>{
  ...
  const NAV=[{id:"semana",icon:"📅",label:"Semana"},{id:"treino",icon:"🏋",label:"Treino"},{id:"coach",icon:"💬",label:"Coach"},{id:"nutri",icon:"🥗",label:"Nutrição"},{id:"config",icon:"⚙️",label:"Config"}];
              ...
              <div>
                ...
                {selDay.bloco?.aquecimento&&(
                  <div style={{marginBottom:16}}>
                    ...
                    {selDay.bloco.aquecimento.atividades.map((a: string,i: number)=>(
                ...
                {principal.length>0&&(
                  <>
                    ...
                    {principal.map((ex: unknown,i: number)=><ExCard key={i} ex={ex} di={dayIdx!}/>)}
                  </>
                )}
                {isRun&&(
                  <div style={{background:T.blDk,border:`1px solid ${T.bl}44`,borderRadius:18,padding:"18px 18px"}}>
                    ...
                    {[["Duração",selDay.bloco.principal.duracao_min&&`${selDay.bloco.principal.duracao_min} min`],["Distância",selDay.bloco.principal.distancia_km&&`${selDay.bloco.principal.distancia_km} km`],["Pace alvo",selDay.bloco.principal.pace_alvo],["BPM alvo",selDay.bloco.principal.bpm_alvo]].filter(([,v])=>v).map(([l,v])=>(
                      <div key={String(l)} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${T.br}`}}>
                ...
                {core.length>0&&(
                  <>
                    ...
                    {core.map((ex: unknown,i: number)=><ExCard key={i} ex={ex} di={dayIdx!}/>)}
                  </>
                )}
                {selDay.bloco?.desaquecimento&&(
                  <div style={{marginTop:16}}>
                    ...
                    {selDay.bloco.desaquecimento.atividades.map((a: string,i: number)=>(
                      <div key={i} style={{fontSize:13,color:T.sub,padding:"5px 0 5px 12px",borderLeft:`2px solid ${T.br2}`,marginBottom:2}}>{a}</div>
                    ))}
                  </div>
                )}
